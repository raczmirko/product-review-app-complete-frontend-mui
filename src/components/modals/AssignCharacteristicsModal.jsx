...
const AssignCharacteristicsModal = ({
  categoryId,
  closeFunction,
  isOpen,
  setIsOpen,
  ...
  const modifyEntity = async (newCategory) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/category/${newCategory.id}/modify`,
        {
          method: "PUT",
          headers,
  ...
  useEffect(() => {
    ...
    if (categoryId !== undefined) {
      ...
      CharacteristicService.listInheritedCharacteristics(categoryId).then(
        (data) => {
        ...
        >
          ...
          >
            <Typography variant="h5" component="div" gutterBottom>
              Assign characteristics to '{category.name}'
            </Typography>
            <Typography variant="subtitle2" component="div" gutterBottom>
              Characteristics are inherited from parent categories. It is not
              possible to assign characteristics that are already inherited, or
              that are already assigned to one of the subcategories, thus these
              don't appear in the list.
            </Typography>
            <hr />
            <Typography variant="h6" component="div" gutterBottom>
              {category.name}'s inherited characteristics:
            </Typography>
            <Box
              sx={{
                maxHeight: 200,
