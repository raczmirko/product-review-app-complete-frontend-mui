...
const CreateProductModal = ({
  ...
  const handleCreate = async () => {
    ...
    if (!purchaseCountry)
      newErrors.purchaseCountry = "Purchase country is required";
    setErrors(newErrors);
    ...
    try {
      const response = await createReviewFunction(
        product,
        description,
        recommended,
        valueForPrice,
          ...
          <Box>
            <Box sx={{ display: "block" }}>
              ...
              <Typography variant="h6" sx={{ mb: 2 }}>
                In what country did you buy this product?
              </Typography>
              <CountrySelector
          ...
          <Box>
            {reviewAspects.length > 0 ? (
              <Typography variant="h6" gutterBottom>
                How would you rate the product's...
              </Typography>
            ) : (
              <Typography variant="h6" gutterBottom>
                There are no review aspects associated to the parent categories.
              </Typography>
            )}
            {reviewAspects.length > 0 &&
              reviewAspects.map((aspect) => (
                ...
                >
                  ...
                  <Rating
                    ...
                    value={aspect.score}
                    onChange={(e, newValue) =>
                      assignScoreToAspect(aspect.id, newValue)
                    }
                  />
                </Box>
              ))}
    ...
    >
      ...
      >
        <Typography variant="h5" component="div" gutterBottom>
          Create a review of "{product.article.name}"
        </Typography>
        <hr />
        <Typography variant="subtitle1" component="div" gutterBottom>
          Create an overall review additionally with review by aspects. Aspects
          are inherited from parent categories.
        </Typography>
        <hr />
        <ToggleButtonGroup
          value={selectedPage}
        ...
        <Box sx={{ minHeight: 250, mt: 2 }}>{renderSelectedPage()}</Box>
